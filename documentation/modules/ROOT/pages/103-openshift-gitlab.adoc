...
[Note]
====
NOTE: このモジュールでは、OpenShiftの管理者アカウント( `kubeadmin` や `cluster-admin` など)を利用します。
====
...
デフォルトで表示されるYAMLからの主な変更点は次の通りです。

- テスト構成なので、`maxReplicas` を `1` に設定して、冗長構成を排除
...
https://docs.redhat.com/ja/documentation/openshift_container_platform/4.20/html/ingress_and_load_balancing/routes[Route]を利用
...
以下の画像の例では `https://gitlab.apps.sno01.sandbox2401.opentlc.com/` にアクセスします。
...
「プロジェクトを作成」からコードのリポジトリを作成できるようになっています。
