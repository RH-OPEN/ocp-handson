...
「コンソール」タブでは、コンテナ内のbashシェルを利用できます。
`id`や`ls`などの適当なLinuxコマンドを実行して、コンテナ内の情報を確認してみてください。
...
`dnf` コマンドでアプリケーションのインストールが完了したら、コンテナのコンソールで次のコマンドを実行します。
...
- コマンド: `/usr/sbin/httpd -D FOREGROUND`
...
新しく実行されたコンテナの「コンソール」タブから `/usr/sbin/php-fpm` コマンドを実行します。
これによって、先ほどの「happy_lamport」コンテナの時と同様に　`curl` コマンドで、Webサービスへのアクセス確認ができるようになります。
