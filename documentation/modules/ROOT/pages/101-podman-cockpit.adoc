...
イメージの検索画面のテキストフィールドに「ubi10」などを入力して、以下のコンテナイメージ選択します。
その後「ダウンロード」ボタンをクリックして、ubi10のコンテナイメージをダウンロードします。
...
なお、コンテナが利用するポートは、コンテナ間で重複しても問題ありません。
====
...
NOTE: この時、「インテグレーション」タブから、ホストポート(例: `8080` )とコンテナポート( `80` )のマッピング指定を忘れないようにしてください。
