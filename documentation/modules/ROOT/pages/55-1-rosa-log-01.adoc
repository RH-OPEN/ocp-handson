...
このとき、上記手順で作成したIAMロールとAmazon S3のBucketを利用するように設定します。


これらを順番に見ていきましょう。

=== Amazon S3をAWS STSで利用するためのIAMロールの作成

AWSのコンソールまたはAWS CLIで、Amazon S3をAWS STSで利用するためのIAMロールを作成します。
IAMロール作成手順は、
https://docs.aws.amazon.com/ja_jp/IAM/latest/UserGuide/id_roles_create_for-custom.html[「カスタム信頼ポリシーを使用してロールを作成する」]を
ご参照ください。

例として、下記画像のような「AmazonS3FullAccess」IAMポリシーを割り当てたIAMロールを作成します。
...
紐づけられたOpenShiftやROSAクラスター上の `openshift-logging` プロジェクトにある `logging-loki` サービスアカウントに対して、IAMポリシーで指定されたAWSリソースへの特定の操作を許可するためのものとなります。後述するLoki OperatorでLokiStackインスタンスを作成する際に、LokiStackインスタンスの名前と同じ名前のサービスアカウントが自動作成されるようになっており、Amazon S3の利用権限をこのサービスアカウントに付与する必要があります。
...
ここで作成したバケット名とバケットのリージョンをメモしておきます。

=== OpenShift Loggingに必要となるOperatorのインストール

OpenShift Loggingを利用するためのOperatorを `cluster-admin` ユーザーで順次インストールしていきます。
...
インストールには、全てデフォルトのパラメータを利用します。
このOperatorは、OpenShiftコンソールでの集約ログ表示設定に利用します。
...
インストール時に、「ロール ARN」に前述の手順で作成したカスタム信頼ポリシー付きのIAMロールのARNを指定します。「更新の承認」はデフォルトは「手動」となっていますが、Operatorの自動更新を有効化したい場合は「自動」を選択してください。
...
vectorとLokiを利用したクラスターロギングを設定します。
...
`UIPlugin` カスタムリソースを作成して、OpenShiftコンソールでの集約ログ表示を有効化します。
ここで指定している `logging-loki` は、前述の手順で作成したLokiStackインスタンスの名前となります。
...
この5つのコマンドは、`cluster-admin` ユーザーで実行します。前の演習で紹介したOpenShiftのWeb Terminalを利用することができます。
