...
+
image::virt/2025_summer/module-05-bcdr/00_Left_Menu.png[link=self, window=blank, width=100%]

. Operatorをクリックして詳細を表示します。

. 利用可能な *Provided APIs* を確認します。 このモジュールでは、 *Backup* および *Restore* 機能を使用します。
+
image::virt/2025_summer/module-05-bcdr/01_Overview.png[link=self, window=blank, width=100%]

. 上部にある水平スクロールバーを使用して、*DataProtectionApplication* タブに移動します。 このオブジェクトは、デプロイされた OADP インスタンスの構成を表します。
+
image::virt/2025_summer/module-05-bcdr/02_DPA.png[link=self, window=blank, width=100%]

. *oadp-dpa* をクリックして _DataProtectionApplication_ の詳細を表示し、次に上部にある *YAML* ボタンをクリックして、設定方法を確認します。
+
image::virt/2025_spring/module-05-bcdr/03_OADP_YAML.png[link=self, window=blank, width=100%]
+
...
+
image::virt/2025_summer/module-05-bcdr/04_Backup_Tab.png[link=self, window=blank, width=100%]
...
+
image::virt/2025_spring/module-05-bcdr/05_Create_Backup_YAML.png[link=self, window=blank, width=100%]
+
...
+
image::virt/2025_summer/module-05-bcdr/06_Backup_Completed.png[link=self, window=blank, width=100%]
...
+
image::virt/2025_spring/module-05-bcdr/07_Fedora02_Overview.png[link=self, window=blank, width=100%]

. 仮想マシンを停止し、停止したら *Actions* ドロップダウンをクリックし、VMを *Delete* するオプションを選択します。
+
image::virt/2025_spring/module-05-bcdr/08_Delete_VM.png[link=self, window=blank, width=100%]

. プロンプトが表示されたら、仮想マシンの削除を確認する赤い *Delete* ボタンをクリックします。
+
image::virt/2025_spring/module-05-bcdr/09_Confirm_Delete.png[link=self, window=blank, width=100%]

. 仮想マシンがインベントリから消えます。
+
image::virt/2025_spring/module-05-bcdr/10_Deleted_VM.png[link=self, window=blank, width=100%]
...
+
image::virt/2025_summer/module-05-bcdr/11_Restore_Tab.png[link=self, window=blank, width=100%]
...
. 一番下の *Create* ボタンをクリックします。
+
image::virt/2025_spring/module-05-bcdr/12_Create_Restore_YAML.png[link=self, window=blank, width=100%]

. *Status* 列が *Completed* に変わったら完了です。
+
image::virt/2025_summer/module-05-bcdr/13_Restore_Completed.png[link=self, window=blank, width=100%]
...
+
image::virt/2025_spring/module-05-bcdr/14_VM_Restored.png[link=self, window=blank, width=100%]
