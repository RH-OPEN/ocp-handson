...
NOTE: このモジュールを実行するためには、東京リージョン(ap-northeast-1)のAmazon Bedrockで、Claude 3の基盤モデルへのアクセスを許可するように設定する必要があります。Amazon Bedrock基盤モデルへのアクセス管理ガイドは、
https://docs.aws.amazon.com/ja_jp/bedrock/latest/userguide/model-access.html[こちらのWebページ]をご参照ください。
...
を利用するように変更
* モデルIDをClaude 3 Haikuに変更 (東京リージョンで利用可能な他のClaude 3のモデルIDも参考として記載)
